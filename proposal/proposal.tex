...
procedure). This is an important practical asset for the proposed work. 
...
\section{Proposed work : From Indexed to Dependent Type Theory}
...
\subsection{A sketch of the design plan}
...
parameterizing it by a signature. I propose that by introducing a
...
substitution such as Beluga and Abella. Beluga's approach is grounded
in ideas from modal logic, and Abella's approach is grounded in proof
theory and cut elimination. What I am proposing is apparently less
principled in this sense (or perhaps the principles have simply not been
discovered yet), but it does seem to provide a clear
...
(completeness, soundness, and termination). This in fact requires fairly deep theory
 ...
 prove consistency and normalization as byproducts.

One objection to such a project is that the resulting theory
...
\LONGVERSION{This section and the next are still really drafty}
...
The additional power of dependent type theory (relative to indexed
types) really appears to be essential for these proofs.
...
If it turns out that such proofs are infeasible for some reason, there
...
\LONGVERSION{
Citations? parametricity \citep{Reynolds,morerecent?}, (step-indexed) logical relations for contextual
  equivalence \citep{Dreyer11,others}, soundness and completenes of
  NbE \citep{Dybjer00,Abel07}
}
