...
% \mbox{Type Constants} & F & \bnfas & \m{Unit} \bnfalt \arrow \bnfalt \times
%                             \bnfalt \Pi \bnfalt \Sigma \bnfalt \mu \\
% \mbox{Normal Types} & T  & \bnfas & \Lambda Z.T \bnfalt \lambda X.T 
%                                     \bnfalt \dsum{\wvec{l : T}}  \bnfalt Q\\
% &&& \bnfalt C_1 = C_2 \andalso T \bnfalt U \\
% \mbox{Neutral Types} & Q & \bnfas & F \bnfalt Z \bnfalt Q\;T \bnfalt \bnfalt Q\;C 
...
\bnfalt C_1 = C_2 \andalso T
\\
&&&    
 \bnfalt \mu Z.\,\mlam {\vec X}  T 
 \bnfalt T\;{\vec C} 
% \bnfalt T\;C
\bnfalt U
...
Case $A = \const i$: by Lemma \ref{lem:haltsclosed}.

Case $A = B \arrow C$: by definition of $\mathcal{R}_A$ we have that $M'$
halts. So by Lemma \ref{lem:haltsclosed} we have that
$M$ halts.

Suppose we are given $N \in \mathcal{R}_B$. Then by rule
$\mathsf{app}$ we have that $M\; N \stepsto M'\; N$. By definition of
$\mathcal{R}_{B \arrow C}$ we have that $M'\;N \in \mathcal{R}_C$. By
the induction hypothesis, $M\;N \in \mathcal{R}_C$.

Hence $M \in \mathcal{R}_{B \arrow C}$.
\end{proof}
}
...
\section{Timeline}
\paragraph{Stage 1} Next 6 months (May 2014 to Nov 2014): working out the metatheory and examples of computation in
types. Optimistically, a POPL paper in July. ESOP deadline in
October. 

\paragraph{Stage 2} 6 months (Nov 2014 to May 2014): prototype implementation and implementing examples
...
\paragraph{Stage 3} 6 months (May 2015 to Nov 2015): writing thesis.
