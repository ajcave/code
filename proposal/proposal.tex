...
\end{alltt}
\LONGVERSION{More meaningful names}
...
\LONGVERSION{Plan is to concentrate on Beluga: contextual types, pair
  contexts with terms. More explanation of Beluga. Why Beluga is
  better for our setting: first class contexts?}

\section{Summary of progress to date}
\LONGVERSION{Explain more in terms of progress than papers?}
...
and (partial) verification. \LONGVERSION{We have illustrated this:
  Olivier's type preserving closure conversion}
...
\LONGVERSION{Use more consistent syntax. Uppercase everywhere}
\begin{lstlisting}
...
\LONGVERSION{Fix syntax}
\begin{lstlisting}
infer : $\Pi$($\Gamma$:tmctx). tm $\Gamma$ -> (tp |$\Gamma$|) option
\end{lstlisting}

However, in the theory described in this paper, one must
instead write a type signature like the following, where we must
express the mapping as a relation and later prove it is total.

\begin{lstlisting}
infer : $\Pi$($\Gamma$:tmctx). $\Pi$($\Delta$:tpctx). ctx_rel $\Gamma$ $\Delta$ -> tm $\Gamma$ -> (tp $\Delta$) option
\end{lstlisting}
...
\begin{enumerate}
\item $[\sigma,N/x]M \in \mathcal{R}_B$ (by I.H. \LONGVERSION{Since
    extending $\sigma$ is reducible})
...
\end{proof}
\LONGVERSION{In a system like Abella, would have to describe
  sim. sub. explicitly}
...
\LONGVERSION{Turn hash S into $\sigma$. Maybe use turnstiles, not g}
\begin{lstlisting}
...
parameterizing it by a signature. I propose \LONGVERSION{Better to say
  explored; want to do it a different way} that by introducing a
(first-order, say de Bruijn) encoding of LF into a dependent type
theory, in much the same way as the intrinsically typed encoding in
Sec. \ref{sec:background} we effectively obtain the virtues of LF in
dependent type theory.

There are two approaches to introducing new types
...
might. Technically, this is not so
different from the other language-based approaches to support
substitution as found in Beluga. Beluga's approach is grounded
in ideas from modal logic, and Abella's approach is grounded in proof
theory and cut elimination \LONGVERSION{...}. \LONGVERSION{What I am proposing is apparently less
principled in this sense (or perhaps the principles have simply not been
discovered yet), but TAKE OUT} it does seem to provide a clear
...
the POPL'12 and LFMTP'13 work, and elaborating the draft on algorithmic
