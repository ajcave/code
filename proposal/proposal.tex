...
\begin{document}
\maketitle
% \abstract{Increasingly, there is a desire to write machine checked proofs of results in
% programming languages and logic. However, giving a formal treatment of the notions of variable binding and substitution has proven to be a significant obstacle towards this goal. This has spawned a large body of work on theories and systems to relieve this particular burden. These systems tend to come at a steep price: they implement fairly weak reasoning logics, which suffice to formalize basic results in programming language theory, but more advanced, state-of-the-art results remain out of reach of such systems. On the other hand, dependent type theory is rich enough to serve as a foundation of mathematics, but provides no direct support for binding and substitution. My proposed thesis aims to support the claim that building a logical framework into dependent type theory is both feasible and makes mechanization of state-of-the-art programming language metatheory more effective. I describe my work to date towards this goal on indexed type systems, including work on indexed recursive types and support for an equational theory of substitutions, and I describe a plan for the future to transition to the dependently typed setting, and applications to formalizing advanced proofs such as normalization.
% }
...
statements, i.e. statements of the form $\forall \vec{x} \exists \vec{y}. \psi$
...
For example, \lstinline{compositional} says that the composition of applying
...
type \lstinline{[$\Psi$ $\vdash$ exp A]}, which we can use as an index to
a computation level type in order to reason
about it \citep{Pientka:2008,Nanevski:2008}. This means that one has direct access to inspect and manipulate
...
type former of the index language: we write \lstinline{[$\Psi$ $\vdash$ $\Phi$]} for the
type of substitutions which provide, for each variable in the context
\lstinline{$\Phi$}, a term in context \lstinline{$\Psi$}.
...
\lstinline{[$\vdash$ $\psi$]}, i.e. taking variables in \lstinline{$\psi$} to closed
...
(implicitly at type \lstinline{[$\vdash$ $\psi$]}) and \lstinline{M} is a reducible
term at type \lstinline{A}, then \lstinline{$\sigma$} with \lstinline{M}
appended is a reducible substitution (implicitly at type
\lstinline{[$\vdash$ $\psi$,x:tm A]} -- the domain has been extended with a
variable of type \lstinline{A}).

\begin{lstlisting}
datatype RedSub : ($\psi$:ctx){$\sigma$:[$\vdash$ $\psi$]} ctype =
| Nil : RedSub [$\vdash$ ^$\,$ ]
...
\begin{lstlisting}
rec fund : {$\psi$:ctx}{M:[$\psi$ $\vdash$ tm A]} RedSub [$\vdash$ $\sigma$] -> Reduce [$\vdash$ A] [$\vdash$ M $\sigma$] =
mlam $\psi$ => mlam M => fn rs => case M of
| [$\psi$ $\vdash$ lam (\x. M1)] =>
  Arr [$\vdash$ h/val s/refl val/lam]
   (mlam N => fn rN => closed [$\vdash$ s/beta]
    (fund [$\psi$,x] [$\psi$,x $\vdash$ M1] (Cons rs rN)))
...
\end{lstlisting}

Weak normalization is now a trivial corollary, taking \lstinline{M} to be a closed term and
\lstinline{$\sigma$} to be the empty substitution:
...
term \lstinline{fund [$\psi$,x] [$\psi$,x $\vdash$ M1] (Cons rs rN)} proves
