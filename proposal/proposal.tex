...
applications to logical relations proofs, it remains to extend
such a theory to truly dependent types. My work to date was important in
understanding 

This is similar in some regards to Hybrid \citep{Felty12}, although I propose
direct integration of the logical framework into the (meta)language in order to provide
more direct support.

One objection to such a project is that the resulting theory
would be quite complex and specialized. My main defense is
that my work on developing logical relations with these tools
indicates that such a system is likely to have significant
...
understood more generally. In the meantime, I will settle
for practical value.

\paragraph{Conversion in type theory}
...
logic (e.g. Abella). indexed types mean 
...
I've prototyped this in Agda, became unfeasible: time to do it
properly.

And we still have to apply these lemmas manually and often compose
several of them
...
provides ``simple'' approach to semantics, termination (via eliminator
for LF datatype)?

A side product of this work will be an explanation of how
