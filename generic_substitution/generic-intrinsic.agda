module generic-intrinsic where
...
data code (A : Set) : Set₁ where
 say : (T : A) -> code A
 ask_*_ : (T : A) -> (C : code A) -> code A
 _⊕_ : (C D : code A) -> code A
 _⊗_ : (C D : code A) -> code A
 Π : (D : A -> code A) -> code A
 _⇒_ : (T : A) (C : code A) -> code A
...
⟦_⟧ : ∀ {A} (C : code A) -> (ctx A -> A -> Set) -> (ctx A -> A -> Set)
⟦_⟧ (say T') X ψ T = T ≡ T'
⟦_⟧ (ask T' * C) X ψ T = (X ψ T') × (⟦ C ⟧ X ψ T)
⟦_⟧ (C ⊕ D) X ψ T = (⟦ C ⟧ X ψ T) ⊎ (⟦ D ⟧ X ψ T)
⟦_⟧ (C ⊗ D) X ψ T = (⟦ C ⟧ X ψ T) × (⟦ D ⟧ X ψ T)
⟦_⟧ (Π D) X ψ T = ∃ (λ S → ⟦ D S ⟧ X ψ T)
⟦_⟧ (T' ⇒ C) X ψ T = ⟦ C ⟧ X (ψ , T') T

data Data {A} (D : code A) (ψ : ctx A) (T : A) : Set where
 ▹ : var ψ T -> Data D ψ T
 ⟨_⟩ : ⟦ D ⟧ (Data D) ψ T -> Data D ψ T

tsubst : ∀ {A} (C : code A) (ψ φ : ctx A) -> Set
tsubst C ψ φ = gsubst ψ (Data C φ)

mutual
 rn : ∀ {A} {C : code A} {T} {ψ φ} -> vsubst ψ φ -> Data C ψ T -> Data C φ T
 rn σ (▹ y) = ▹ (σ y)
 rn {A} {C} σ ⟨ M ⟩ = ⟨ rn2 C σ M ⟩

 rn2 : ∀ {A} (C : code A) {D} {ψ φ} {T} -> vsubst ψ φ
   -> ⟦ C ⟧ (Data D) ψ T  -> ⟦ C ⟧ (Data D) φ T
 rn2 (say T) σ M = M
 rn2 (ask T * C) σ M = {!rn !} , {!!}
 rn2 (C ⊕ D) σ M = {!!}
 rn2 (C ⊗ D) σ M = {!!}
