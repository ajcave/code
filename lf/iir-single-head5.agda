...
mutual
 ...
 lookup ⊡ ()
 lookup (Γ ,' T) top = [ wkn-vsubst ]tv T
 lookup (Γ ,' T) (pop x) = [ wkn-vsubst ]tv (lookup Γ x)
 ...
 hlookup {Σ} Γ (con c) = [ unit ]tv (lookupsτ Σ c)
...
-- [_]isv : ∀ {Σ} {Γ Δ : ctx Σ} {K : kind Γ} -> (σ : vsubst Γ Δ) -> inSig Σ K -> inSig Σ ([ σ ]kv K)
 ...
 data eqDec {Σ} {Γ : ctx Σ} (Δ : ctxext Γ) : var (Γ << Δ) -> Set where
  before : (y : var Γ) -> eqDec Δ ([ build-sub Γ Δ ⊡ ]v1 y)
 ...
 var-eq {Σ} {Γ} (Δ , T) (pop .([ build-sub Γ Δ ⊡ ]v1 y)) | before y = subst (eqDec (Δ , T)) trustMe (before y)
