...
mutual
 ...
 data kind {Σ : sig} : Set where
  ⋆ : kind
 
 data tp (Σ : sig) : Set where
  Π : (T : tp Σ) (S : tp Σ) -> tp Σ
  · : (x : inSig1κ Σ) {- -> tpSpine Γ ([ unit ]kv (lookups Σ x)) -} -> tp Σ
...
data ctxext {Σ} (Γ : ctx Σ) : Set where
 ⊡ : ctxext Γ
 _,_ : (Δ : ctxext Γ) -> (T : tp Σ) -> ctxext Γ
 ...
 -- vsubst ⊡ Δ = Unit
 -- vsubst (Γ ,' T) Δ = Σ (vsubst Γ Δ) (λ σ -> Σ (var Δ) (λ x -> lookup Δ x ≡ T))
...
mutual
 ...
 spine : ∀ {Σ} (Γ : ctx Σ) -> tp Σ -> tp Σ -> Set
 spine Γ (Π A A₁) B = Σ (ntm Γ A) (λ N -> spine Γ A₁ B)
 spine Γ (· c) B = (· c) ≡ B
...
--  -- ⇑n : ∀ {Σ} (Γ : ctx Σ) (Δ : ctxext Γ) (Δ' : ctxext Γ) {T} -> ntm (Γ << Δ') T -> ntm ((Γ << Δ) << (⇑ce Γ Δ Δ')) ([ build-sub Γ Δ Δ' ]tv T)
...
--  var-eq {Σ} {Γ} (Δ , T) (pop ._) | before y = before y
