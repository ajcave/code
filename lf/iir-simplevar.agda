...
mutual
 ...
 lookup ⊡ ()
 lookup (Γ ,' T) top = [ wkn-ntsubst ]tpn T
 lookup (Γ ,' T) (pop x) = [ wkn-ntsubst ]tpn (lookup Γ x)
 ...
 data ntm {Σ} (Γ : ctx Σ) : tp Γ -> Set where
  _·_ : ∀ {T} {a : inSig1κ Σ} {S} -> head Γ T -> spine Γ T (a · S) -> ntm Γ (a · S)
 ...
 _◆'_ : ∀ {Σ} {Γ : ctx Σ} {a : inSig1κ Σ} {T : tp Γ} {S} -> ntm Γ T -> spine Γ T (a ·' S) -> ntm Γ (a ·' S)
 (H · S) ◆' S₁ = H · (S ++ S₁)
 ƛ N ◆' (N₁ , S) = ([ N₁ /x]nn N) ◆' S
