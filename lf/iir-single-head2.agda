...
mutual
 ...
 data tvar {Σ} (Γ : ctx Σ) (T : tp Γ) : Set where
  _,_ : (x : var Γ) -> lookup Γ x ≡ T -> tvar Γ T
 ...
 do-wkn-vsubst {Σ} {Γ ,' T} (σ , x) = (do-wkn-vsubst σ) , ? --(subst (varOpt_) trustMe (pop x)) 
