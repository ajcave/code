...
mutual
 ...
 ⇑tk : ∀ {Σ} (Γ : ctx Σ) (Δ : ctxext Γ) -> tp Γ -> tp (Γ << Δ)
 ⇑tk Γ Δ T = ⇑t Γ Δ ⊡ T
 ...
 ⊡ce : ∀ {Σ} {Γ : ctx Σ} -> ctxext Γ
 ⊡ce = ⊡

 _+_ : ∀ {Σ} {Γ : ctx Σ} B (Δ : ctxext (Γ ,, B)) -> ctxext Γ
 B + ⊡ = ⊡ , B
 ...
 nv-sub : ∀ {Σ} {Γ : ctx Σ} {B} (N : ntm Γ B) (Δ : ctxext (Γ ,, B)) (Δ' : ctxext (Γ << (n-cesub N Δ)))
       (x : var1 ((Γ ,, B) << Δ)) {a St}
   -> spine ((Γ << (n-cesub N Δ)) << Δ') (⇑tk (Γ << n-cesub N Δ) Δ' (n-tsub N Δ (lookup ((Γ ,, B) << Δ) x))) (a ·' St)
   -> ntm ((Γ << (n-cesub N Δ)) << Δ') (a ·' St)
