...
mutual
 ...
 ⟦_⊢_∶_⟧2 : ∀ {n} {Γ : dctx n} (Γwf : wfctx Γ) {M : tm n} {T : tm n} -> (d : Γ ⊢ M ∶ T) -> (Twf : Γ ⊢ T type) -> (γ : ⟦ Γwf ⟧c)
   -> ⟦ Γwf ⊢ Twf  ⟧t γ
 ⟦_⊢_∶_⟧2 Γ₁ bool set γ = Bool
 ⟦_⊢_∶_⟧2 Γ₁ bool (emb x) γ = {!!}
 ⟦_⊢_∶_⟧2 Γ₁ tt (emb bool) γ = true
 ⟦_⊢_∶_⟧2 Γ₁ tt (emb (conv x x₁ x₂)) γ = {!!}
 ⟦_⊢_∶_⟧2 Γ₁ ff T γ = {!!}
 ⟦_⊢_∶_⟧2 Γ₁ (▹ x₁ x₂) T γ = {!!}
 ⟦_⊢_∶_⟧2 Γ₁ (Π M M₁) set γ = (x : ⟦ Γ₁ ⊢ M ∶ set ⟧2 γ) → ⟦ Γ₁ , emb M ⊢ M₁ ∶ set ⟧2 (γ , x)
 ⟦_⊢_∶_⟧2 Γ₁ (Π M M₁) (emb x) γ = {!!}
 ⟦_⊢_∶_⟧2 Γ₁ (ƛ x M₁) (Π T T₁) γ = λ x₁ → ⟦ Γ₁ , T ⊢ M₁ ∶ T₁ ⟧2 (γ , x₁)
 ⟦_⊢_∶_⟧2 Γ₁ (ƛ x M₁) (emb (Π x₁ x₂)) γ = λ x₃ → {!!}
 ⟦_⊢_∶_⟧2 Γ₁ (ƛ x M₁) (emb (conv x₁ x₂ x₃)) γ = {!!}
 ⟦_⊢_∶_⟧2 Γ₁ (M₁ · M₂) T γ = {!!}
 ⟦_⊢_∶_⟧2 Γ₁ (if x M₁ M₂ M₃) T γ = {!!}
 ⟦_⊢_∶_⟧2 Γ₁ (conv x x₁ M₁) T γ = {!!}