...
vsub-ext : vsubst -> vsubst
vsub-ext σ zero = zero
vsub-ext σ (suc n) = suc (σ n)
...
tsub-ext : tsubst -> tsubst
tsub-ext σ zero = ▹ zero
tsub-ext σ (suc n) = [ suc ]r (σ n)
...
postulate
 ren⟶*' : ∀ (σ : vsubst) {M N} -> M ⟶ N -> [ σ ]r M ⟶ [ σ ]r N
 ren⟶* : ∀ (σ : vsubst) {M N} -> M ⟶* N -> [ σ ]r M ⟶* [ σ ]r N
 sub⟶*' : ∀ (σ : tsubst) {M N} -> M ⟶ N -> [ σ ]t M ⟶ [ σ ]t N
 sub⟶* : ∀ (σ : tsubst) {M N} -> M ⟶* N -> [ σ ]t M ⟶* [ σ ]t N
 ren-ext-comp : ∀  {w : vsubst} {w' : vsubst } B
   -> [ vsub-ext (w' ∘ w) ]r B ≡ [ vsub-ext w' ]r ([ vsub-ext w ]r B)
 sub⟶*2 : ∀  {M N : tm} {σ : tsubst} -> M ⟶* N -> ∀ (P : tm) -> [ σ ,, M ]t P ⟶* [ σ ,, N ]t P
 subeq3 : ∀ {σ : tsubst} M {N} -> [ σ ]t M ≡ [ σ ,, N ]t ([ wkn-vsub ]r M)
 subeq1 : ∀  {σ : tsubst} M {N} -> [ σ ,, ([ σ ]t N) ]t M ≡ [ σ ]t ([ N /x] M) 
 --subeq2' : ∀ {n m} {w : vsubst} M {N} -> [ ([ id-tsub ]tr w) , N ]t M ≡ [ id-tsub , N ]t ([ vsub-ext w ]r M)
 subeq2 : ∀  {σ : tsubst} M {N} -> [ σ ,, N ]t M ≡ [ id-tsub ,, N ]t ([ tsub-ext σ ]t M)
 subeq4 : ∀ {M : tm} -> [ id-tsub ]t M ≡ M
 reneq4 : ∀ {M : tm} -> [ id-vsub ]r M ≡ M
 reneq4' : ∀  {σ : tsubst} -> [ id-vsub ]rs σ ≡ σ
 rename-norm : ∀  {w : vsubst} {A} -> normal A -> normal ([ w ]r A)
 rename-neut : ∀  {w : vsubst} {A} -> neutral A -> neutral ([ w ]r A)
 ren-comp : ∀  {w : vsubst} {v : vsubst} M -> [ w ]r ([ v ]r M) ≡ [ w ∘v v ]r M
 ren-sub-comp : ∀  {w : vsubst } {σ : tsubst} M -> [ w ]r ([ σ ]t M) ≡ [ [ w ]rs σ ]t M
 sub-ren-comp : ∀  {σ : tsubst } {w : vsubst} M -> [ σ ]t ([ w ]r M) ≡ [ [ σ ]tr w ]t M
 sub-ren-ext-comp :  ∀  {σ : tsubst } {w : vsubst}
  -> ([ tsub-ext σ ]tr (vsub-ext w)) ≡ (tsub-ext ([ σ ]tr w))
 ren-sub-ext-comp :  ∀  {w : vsubst } {σ : tsubst}
  -> ([ vsub-ext w ]rs (tsub-ext σ)) ≡ (tsub-ext ([ w ]rs σ))
 subeq6 : ∀ (B : tm) {N} {w : vsubst} -> [ N /x] ([ vsub-ext w ]r B) ≡ [ [ id-tsub ]tr w ,, N ]t B
 subeq5 : ∀ {σ : tsubst} {N} {w : vsubst} -> [ σ ,, N ]tr (suc ∘ w) ≡ [ σ ]tr w

-- top = zero
-- pop = suc

postulate
 subeq7 : ∀ {B : tm} -> [ id-tsub ,, ▹ zero ]t ([ vsub-ext wkn-vsub ]r B) ≡ B
 {-subeq7 {n} {B} = trans
 (subeq6 B)
 (trans (cong (λ α → [ α , ▹ top ]t B) (trans (gmap-funct id-vsub) (trans (gmap-cong {!!}) (sym (gmap-funct id-vsub))))) subeq4) -}
 
rename-norm-bool : ∀ {w : vsubst} {A} -> normal-bool A -> normal-bool ([ w ]r A)
rename-norm-bool tt = tt
rename-norm-bool ff = ff
rename-norm-bool (neut y) = neut (rename-neut y)

data _≈_ (a b : tm) : Set where
 common : ∀ {d} -> (a ⟶* d) -> (b ⟶* d) -> a ≈ b


⟶*cong2 : ∀ {M1 M2 N1 N2 : tm} -> M1 ≡ M2 -> N1 ≡ N2 -> M1 ⟶* N1 -> M2 ⟶* N2
⟶*cong2 refl refl t = t

postulate
 cr : ∀ {a b c : tm} -> a ⟶* b -> a ⟶* c -> b ≈ c

data pi-inj1-res (A : tm) B : (C : tm) -> Set where
 yep : ∀ {A' B'} -> A ⟶* A' -> B ⟶* B' -> pi-inj1-res A B (Π A' B')

pi-inj1 : ∀ {A : tm} {B C} -> (Π A B) ⟶* C -> pi-inj1-res A B C
...
⟶≈trans' : ∀ {A B C : tm} -> A ≈ B -> A ⟶ C -> C ≈ B
⟶≈trans' t u = ≈-trans (≈-sym (⟶-≈ u)) t

mutual
 neutral-step : ∀ {C : Set} {A B : tm} -> neutral A -> A ⟶ B -> C
 neutral-step (▹ x) ()
 neutral-step (() · x) β
 ...
 normal-step : ∀ {A B : tm} {C : Set} -> normal A -> A ⟶ B -> C
...
mutual
 data Ψ : tm -> Set where
  bool : Ψ bool
  Π : ∀ {A B} -> (p : Ψ A) -> (∀ (w : vsubst) (a : tm) -> ψ p w a -> Ψ ([ a /x] ([ vsub-ext w ]r B))) -> Ψ (Π A B)
 ...
 ψ : ∀ {A : tm} -> Ψ A -> ∀ (w : vsubst) -> tm -> Set
 ψ bool w a = ∃ (λ b → (a ⟶* b) × normal-bool b)
 ψ (Π p f) w a = normalizable a × (∀ (v : vsubst) b (q : ψ p (v ∘v w) b) → ψ (f (v ∘v w) b q) id-vsub ([ v ]r a · b))
 ψ (neut y) w a = ∃ (λ b → (a ⟶* b) × neutral b)
 ψ (closed y y') w a = ψ y' w a

 
 Πc : ∀ {A B} -> (p : Ψ A) -> (∀ (w : vsubst) (a : tm) -> ψ p w a -> Ψ ([ a /x] ([ vsub-ext w ]r B))) -> Ψ (Π A B)
 Πc p y = Π p y

 Ψwkn : ∀ (w : vsubst) {A} -> Ψ A -> Ψ ([ w ]r A)
 Ψwkn w bool = bool
 ...
 ψfunct : ∀ {A} (w : vsubst) (w' : vsubst) (t : Ψ A) {a} -> ψ (Ψwkn w t) w' a -> ψ t (w' ∘v w) a
 ψfunct w w' bool r = r
 ψfunct w w' (Π {A} {B} p y) (r1 , r2) = r1 , f 
  where f : ∀ (v : vsubst) b q -> _
        f v b q = let z0 = (ψfunct' w (v ∘v w') p q) in
                  let q1 = cong [ b /x] (ren-ext-comp {w = w} {w' = (v ∘v w')} B)
                    in ψeqdep (subst Ψ q1 (y ((v ∘v w') ∘v w) b (ψfunct w (v ∘v w') p z0)))
                              (y (v ∘v (w' ∘v w)) b q)
                              (sym (trans (cong (λ α → [ b /x] ([ vsub-ext α ]r B)) refl) q1))
                              (r2 v b z0)

 ψfunct w w' (neut y) r = r
 ψfunct w w' (closed y y') r = ψfunct w w' y' r

 ψfunct' : ∀ {A} (w : vsubst) (w' : vsubst) (t : Ψ A) {a} -> ψ t (w' ∘v w) a -> ψ (Ψwkn w t) w' a
 ψfunct' w w' bool r = r
 ψfunct' w w' (Π {A} {B} p y) (r1 , r2) = r1 , f
  where f : ∀ (v : vsubst) b q -> _
        f v b q = let z0 = ψfunct w (v ∘v w') p q in
                  let z1 = r2 v b z0 in
                    ψeqdep (y (v ∘v (w' ∘v w)) b z0)
                          (subst Ψ (cong [ b /x] (ren-ext-comp B)) (y ((v ∘v w') ∘v w) b (ψfunct w (v ∘v w') p q)))
                          (trans (cong (λ α → [ b /x] ([ vsub-ext α ]r B)) refl) (cong [ b /x] (ren-ext-comp B)))
                          z1

 ψfunct' w w' (neut y) r = r
 ψfunct' w w' (closed y y') r = ψfunct' w w' y' r

 ψfunctid : ∀ {A} (w : vsubst) (t : Ψ A) {a} -> ψ (Ψwkn w t) id-vsub a -> ψ t w a
 ψfunctid w t {a} p = subst (λ α → ψ t α a) refl (ψfunct w id-vsub t p)

 ψfunct'id : ∀ {A} (w : vsubst) (t : Ψ A) {a} -> ψ t w a -> ψ (Ψwkn w t) id-vsub a
 ψfunct'id w t {a} p = ψfunct' w id-vsub t (subst (λ α → ψ t α a) refl p)

 lemma3-3 : ∀ {A B : tm} {M} (p : Ψ A) (q : Ψ B) (w : vsubst) -> A ≈ B -> (ψ p w M -> ψ q w M) × (ψ q w M -> ψ p w M)
 ...
 lemma3-3c : ∀ {A M : tm} (p q : Ψ A) -> ψ p id-vsub M -> ψ q id-vsub M
 lemma3-3c p q t = _×_.proj₁ (lemma3-3 p q id-vsub ≈-refl) t

 ψeqdep : ∀ {B B' M : tm} (p : Ψ B) (q : Ψ B') -> B ≡ B' -> ψ p id-vsub M -> ψ q id-vsub M
 ψeqdep p q refl t = lemma3-3c p q t

Ψ-closed⟶* : ∀ {A B : tm} -> A ⟶* B -> Ψ B -> Ψ A
Ψ-closed⟶* refl t = t
Ψ-closed⟶* (trans1 x s) t = closed x (Ψ-closed⟶* s t)

ψ-closed' : ∀ {A : tm} (w : vsubst) {M N} -> (p : Ψ A) -> M ⟶* N -> ψ p w N -> ψ p w M
...
ψ-closed : ∀ {A : tm} {M N} -> (p : Ψ A) -> M ⟶* N -> ψ p id-vsub N -> ψ p id-vsub M
...
mutual
 data Φ : tm -> Set where
  bool : Φ bool
  Π : ∀ {A B} -> (p : Φ A) -> (∀ (w : vsubst) (a : tm) -> φ p w a -> Φ ([ a /x] ([ vsub-ext w ]r B))) -> Φ (Π A B)
  neut : ∀ {A} -> neutral A -> Φ A
  closed : ∀ {A B} -> A ⟶ B -> Φ B -> Φ A
  set : Φ set

 φ : ∀ {A : tm} -> Φ A -> ∀ (w : vsubst) -> tm -> Set
 φ bool w a = ∃ (λ b → (a ⟶* b) × normal-bool b)
 φ (Π p f) w a = normalizable a × (∀ (v : vsubst) b (q : φ p (v ∘v w) b) → φ (f (v ∘v w) b q) id-vsub ([ v ]r a · b))
 φ (neut y) w a = ∃ (λ b → (a ⟶* b) × neutral b)
 φ (closed y y') w a = φ y' w a
 φ set w a = Ψ a

 Φwkn : ∀ (w : vsubst) {A} -> Φ A -> Φ ([ w ]r A)
 Φwkn w bool = bool
 ...
 φfunct : ∀ {A} (w : vsubst) (w' : vsubst) (t : Φ A) {a} -> φ (Φwkn w t) w' a -> φ t (w' ∘v w) a
 φfunct w w' bool r = r
 ...
 φfunct' : ∀ {A} (w : vsubst) (w' : vsubst) (t : Φ A) {a} -> φ t (w' ∘v w) a -> φ (Φwkn w t) w' a
 φfunct' w w' bool r = r
 φfunct' w w' (Π {A} {B} p y) (r1 , r2) = r1 , f
  where f : ∀ (v : vsubst) b q -> _
        f v b q = let z0 = φfunct w (v ∘v w') p q in
                  let z1 = r2 v b z0 in
                       φeqdep (y (v ∘v (w' ∘v w)) b z0)
 ...
 φfunct'id : ∀ {A} (w : vsubst) (t : Φ A) {a} -> φ t w a -> φ (Φwkn w t) id-vsub a
 φfunct'id w t {a} p = φfunct' w id-vsub t (subst (λ α → φ t α a) refl p)

 lemma3-3' : ∀ {A B : tm} {M} (p : Φ A) (q : Φ B) (w : vsubst) -> A ≈ B -> (φ p w M -> φ q w M) × (φ q w M -> φ p w M)
 lemma3-3' bool bool w s = (λ r -> r) , (λ r -> r)
 lemma3-3' bool (Π p y) w s = bool≈Π s
 lemma3-3' bool (neut y) w s = bool-≈-neutral y s
 lemma3-3' (Π p y) bool w s = bool≈Π (≈-sym s)
 ...
 lemma3-3c' : ∀ {A M : tm} (p q : Φ A) -> φ p id-vsub M -> φ q id-vsub M
 lemma3-3c' p q = _×_.proj₁ (lemma3-3' p q id-vsub ≈-refl)

 φeqdep : ∀ {B B' M : tm} (p : Φ B) (q : Φ B') -> B ≡ B' -> φ p id-vsub M -> φ q id-vsub M
 φeqdep p q refl t = lemma3-3c' p q t

Φ-closed⟶* : ∀ {A B : tm} -> A ⟶* B -> Φ B -> Φ A
Φ-closed⟶* refl t = t
Φ-closed⟶* (trans1 x s) t = closed x (Φ-closed⟶* s t)

φ-closed' : ∀ {A : tm} (w : vsubst) {M N} -> (p : Φ A) -> M ⟶* N -> φ p w N -> φ p w M
...
φ-closed : ∀ {A : tm} {M N} -> (p : Φ A) -> M ⟶* N -> φ p id-vsub N -> φ p id-vsub M
