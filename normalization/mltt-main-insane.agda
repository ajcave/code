module mltt-main-insane where
open import mltt-insane
open import Data.Nat
open import Function

φsubst : ∀ {A A' : tm} (p : Φ A) (e : A ≡ A') {M} {w : vsubst} -> φ p w M -> φ (subst Φ e p) w M
φsubst p refl t = t

φsubst' : ∀ {A A' : tm} (p : Φ A) (e : A ≡ A') {M} {w : vsubst} -> φ (subst Φ e p) w M -> φ p w M
φsubst' p refl t = t

φeqdep' : ∀ {B B' M N : tm} (p : Φ B) (q : Φ B') -> B ≡ B' -> M ⟶ N -> φ p id-vsub N -> φ q id-vsub M
φeqdep' p q refl s t = lemma3-3c' p q (φ-closed p (trans1 s refl) t)

φeqdep2 : ∀ {B B' M N : tm} (p : Φ B) (q : Φ B') -> B ≡ B' -> M ≡ N -> φ p id-vsub N -> φ q id-vsub M
φeqdep2 p q refl refl t = lemma3-3c' p q t

φstep : ∀ {B M N : tm} (p : Φ B) -> M ⟶ N -> φ p id-vsub N -> φ p id-vsub M
φstep p s t = φ-closed p (trans1 s refl) t

φcong : ∀ {B M N : tm} (p : Φ B) -> M ≡ N -> φ p id-vsub N -> φ p id-vsub M
φcong p refl t = t

φeq : ∀ {B B' M N : tm} (p : Φ B) (q : Φ B') -> B' ⟶* B -> M ⟶* N -> φ p id-vsub N -> φ q id-vsub M
...
φwkn : ∀ {A : tm} {w : vsubst} (v : vsubst) (p : Φ A) {M} -> φ p w M -> φ p (v ∘v w) ([ v ]r M)
...
φwkn {w = w} v (Π {A} {B} p y) (norm q r , t2) = norm (ren⟶* v q) (rename-norm r) , λ v' b q ->
                 ...
                 (y ((v' ∘v v) ∘v w) b q)
                 (y (v' ∘v (v ∘v w)) b q)
                 (cong (λ α → [ b /x] ([ vsub-ext α ]r B)) refl)
...
φwkn' : ∀ {A : tm}  (v : vsubst) (p : Φ A) {M} -> φ p id-vsub M -> φ (Φwkn v p) id-vsub ([ v ]r M)
φwkn' v p {M} t = φfunct'id v p (subst (λ α → φ p α ([ v ]r M)) refl (φwkn v p t))


mutual
 Πinv1' : ∀ (A : tm) B -> Φ (Π A B) -> Φ A
 Πinv1' A B (Π p x) = p
 Πinv1' A B (neut ())
 Πinv1' A B (closed (Π1 x) p) = closed x (Πinv1' _ B p)
 Πinv1' A B (closed (Π2 x) p) = Πinv1' A _ p

 Πinv2' : ∀ (A : tm) B -> (p : Φ (Π A B)) -> ∀ {a} -> φ (Πinv1' A B p) id-vsub a -> Φ ([ a /x] B)
 Πinv2' A B (Π p y) {a} q with (y id-vsub a q)
 ... | q0 = {!!}
 Πinv2' A B (neut ()) q
...
mutual
 reflect : ∀ {A M : tm} -> (p : Ψ A) -> neutral M -> ψ p id-vsub M
 reflect bool r = _ , (refl , neut r)
 reflect (Π p y) r = (norm refl (neut r)) , f
  where f : ∀ (v : vsubst) b q -> ψ (y (v ∘v id-vsub) b q) id-vsub ([ v ]r _ · b)
        f v b q with reify (Ψwkn v p) (ψfunct'id v p (subst (λ α → ψ p α b) refl q))
 ...
 reify : ∀ {A M : tm} -> (p : Ψ A) -> ψ p id-vsub M -> normalizable M
 reify bool (t1 , (t2 , t3)) = norm t2 (normal-bool-normal t3)
 reify (Π p y) (h , _) = h
 reify (neut y) (t1 , (t2 , r)) = norm t2 (neut r)
 reify (closed y y') r = reify y' r

reifyt : ∀ {A : tm} -> Ψ A -> normalizable A
reifyt bool = norm refl bool
reifyt (Π t x) with reifyt t | reifyt (x wkn-vsub (▹ zero) (ψfunctid wkn-vsub t (reflect (Ψwkn wkn-vsub t) (▹ zero))))
...
mutual
 reflect' : ∀ {A M : tm} -> (p : Φ A) -> neutral M -> φ p id-vsub M
 reflect' bool r = _ , (refl , neut r)
 reflect' (Π p y) r = (norm refl (neut r)) , f
  where f : ∀ (v : vsubst) b q -> φ (y (v ∘v id-vsub) b q) id-vsub ([ v ]r _ · b)
        f v b q  with reify' (Φwkn v p) (φfunct'id v p (subst (λ α → φ p α b) refl q))
        f v b q | norm y' y0 = φ-closed (y (v ∘v id-vsub) b q) (app2* y') (reflect' (y (v ∘v id-vsub) b q) ((rename-neut r) · y0))
 reflect' (neut y) r = _ , (refl , r)
 reflect' (closed y y') r = reflect' y' r
 reflect' set r = neut r

 reify' : ∀ {A M : tm} -> (p : Φ A) -> φ p id-vsub M -> normalizable M
 reify' bool (t1 , (t2 , t3)) = norm t2 (normal-bool-normal t3)
 reify' (Π p y) (h , _) = h
 reify' (neut y) (t1 , (t2 , r)) = norm t2 (neut r)
 reify' (closed y y') r = reify' y' r
...
sub-ren-lem : ∀ (σ : tsubst) (v : vsubst) (a : tm) B
...
if1* : ∀ {M N1 N1' N2 : tm} -> N1 ⟶* N1' -> (if M N1 N2) ⟶* (if M N1' N2)
if1* refl = refl
if1* (trans1 x t) = trans1 (ifc1 x) (if1* t)

if2* : ∀ {M N1 N2 N2' : tm} -> N2 ⟶* N2' -> (if M N1 N2) ⟶* (if M N1 N2')
if2* refl = refl
if2* (trans1 x t) = trans1 (ifc2 x) (if2* t)

if3* : ∀ {M M' N1 N1' N2 N2' : tm} -> M ⟶* M' -> N1 ⟶* N1' -> N2 ⟶* N2' -> (if M N1 N2) ⟶* (if M' N1' N2')
...
dmap : (tm -> tm) -> dctx -> dctx
dmap f ⊡ = ⊡
dmap f (Γ , x) = (dmap f Γ) , (f x)
...
data _∋_∶_ : (Γ : dctx) -> ℕ -> tm -> Set where
 top : ∀ {Γ A} -> (Γ , A) ∋ zero ∶ A
 pop : ∀ {Γ A B x} -> Γ ∋ x ∶ B -> (Γ , A) ∋ suc x ∶ B

mutual
 wfctx : dctx -> Set
 wfctx Γ = ∀ x A -> Γ ∋ x ∶ A -> Γ ⊢ A type

 data _⊢_type (Γ : dctx) : tm -> Set where
  set : Γ ⊢ set type
  Π : ∀ {A B} -> Γ ⊢ A type -> (↑ (Γ , A)) ⊢ B type -> Γ ⊢ (Π A B) type
  emb : ∀ {A} -> Γ ⊢ A ∶ set -> Γ ⊢ A type

 data _⊢_∶_ (Γ : dctx) : tm -> tm -> Set where
  bool : Γ ⊢ bool ∶ set
  tt : Γ ⊢ tt ∶ bool
  ff : Γ ⊢ ff ∶ bool
...
-- data Φs (σ : tsubst) : dctx -> Set where
--  ⊡ : Φs σ ⊡
--  _,_ : ∀ {Γ A} -> Φs σ Γ -> Φ ([ σ ]t A) -> Φs σ (Γ , A)

-- data φs (σ : tsubst) : (Γ : dctx) -> Φs σ Γ -> Set where
--  ⊡ : φs σ ⊡ ⊡
--  _,[_]_ : ∀ {Γ} {ps} {A} -> φs (σ ∘ suc) Γ ps -> ∀ p -> φ p id-vsub (σ zero) -> φs σ (Γ , A) (ps , p)

Φs : (σ : tsubst) (Γ : dctx) -> Set
Φs σ Γ = ∀ x A → Γ ∋ x ∶ A → Φ ([ σ ]t A)

φs : (σ : tsubst) (Γ : dctx) (ps : Φs σ Γ) -> Set
φs σ Γ ps = ∀ x A -> (p : Γ ∋ x ∶ A) -> φ (ps x A p) id (σ x)

Φswkn : ∀ {Γ : dctx} {σ : tsubst} (w : vsubst) -> Φs σ Γ -> Φs ([ w ]rs σ) Γ
Φswkn w f x A p = subst Φ (ren-sub-comp A) (Φwkn w (f x A p))

J : ∀ {A : Set} {x : A} (P : (y : A) -> x ≡ y -> Set) -> P x refl -> ∀ {y} -> (p : x ≡ y) ->  P y p
J P p refl = p

φswkn : ∀ {Γ : dctx} {σ : tsubst} (w : vsubst) {ps : Φs σ Γ} -> φs σ Γ ps -> φs ([ w ]rs σ) Γ (Φswkn w ps)
φswkn {Γ} {σ} w {ps} f x A p = J (λ y α → φ (subst Φ α (Φwkn w (ps x A p))) id ([ w ]r (σ x))) (φwkn' w (ps x A p) (f x A p)) (ren-sub-comp A)

_⊨_type : ∀ (Γ : dctx) -> tm -> Set
Γ ⊨ A type = ∀ {σ : tsubst} {ps : Φs σ Γ} -> φs σ Γ ps -> Φ ([ σ ]t A)

Φs,, : ∀ {Γ σ a} -> Φs σ Γ -> Φs (σ ,, a) (↑ Γ)
Φs,, {⊡} f x A ()
Φs,, {Γ , A} f 0 .([ suc ]r A) top = subst Φ (subeq3 A) (f zero A top)
Φs,, {Γ , B} f (suc x₁) A (pop p) =
  Φs,, (λ x A₁ x₂ → f (suc x) A₁ (pop x₂)) _ A p

Φs, : ∀ {Γ σ A} -> Φs σ Γ -> Φ ([ σ ]t A) -> Φs σ (Γ , A)
Φs, f t 0 A top = t
Φs, f t (suc x) A₁ (pop p) = f x A₁ p

φs,, : ∀ {Γ σ a} (ps : Φs σ Γ) -> φs σ Γ ps -> φs (σ ,, a) (↑ Γ) (Φs,, ps)
φs,, {⊡} ps qs x A ()
φs,, {Γ , A} {σ} {a} ps qs 0 ._ top = J (λ y α → φ (subst Φ α (ps zero A top)) id a) {!!} (subeq3 A)
φs,, {Γ , A} ps qs (suc x) B (pop p) = {!!}

Π' : ∀ {Γ} (A : tm) B -> Γ ⊨ A type -> (↑ (Γ , A)) ⊨ B type -> Γ ⊨ (Π A B) type
Π' A B t1 t2 {σ = σ} {ps} x = Π (t1 {σ} {ps} x) f
 where f : (w : vsubst) (a : tm) → φ (t1 {σ} {ps} x) w a → Φ ([ a /x] ([ vsub-ext w ]r ([ tsub-ext σ ]t B)))
       f v a x₁ = subst Φ (sub-ren-lem σ v a B) (t2 {([ v ]r ∘ σ) ,, a}
         {Φs,, (Φs, (Φswkn v ps) (subst Φ (ren-sub-comp A) (Φwkn v (t1 {σ} {ps} x))))}
         {!!}
        )

--  (λ v a x₁ → subst Φ (sub-ren-lem σ v a B) (t2 {([ v ]r ∘ σ) ,, a} {Φs,, (Φs, (Φswkn v ps) (subst Φ (ren-sub-comp A) (Φwkn v (t1 {σ} {ps} x))))}
  -- {!!}))
...
mutual
 lem1 : ∀ {A} (Γ : dctx) -> Γ ⊢ A type -> Γ ⊨ A type
 lem1 Γ set = λ x -> set
...
mutual
 idΦ : ∀ {Γ : dctx} -> wfctx Γ -> Φs ▹ Γ
 idΦ {Γ} f x A p with f x A p
 ... | q = {!!} -- This is where we get stuck
 -- idΦ {⊡} f x A ()
 -- idΦ {Γ , B} f 0 .B top with lem1 (Γ , B) (f zero B top) {▹} {
 -- ... | q = {!!}
 -- idΦ {Γ , B} f (suc x) A (pop p) = {!!}
 -- ⊡ = ⊡
 -- idΦ (_,_ d {A} x) = Φswkn wkn-vsub (idΦ d) , subst Φ (ren-sub-comp A) (Φwkn wkn-vsub (lem1 _ x (idφ d))) 

 idφ : ∀ {Γ : dctx} (d : wfctx Γ) -> φs ▹ Γ (idΦ d)
 idφ = {!!}
 -- ⊡ = ⊡
 -- idφ (_,_ d {A} x) = φswkn wkn-vsub (idφ d) ,[ subst Φ (ren-sub-comp A) (Φwkn wkn-vsub (lem1 _ x (idφ d))) ]
...
-- Notes:
-- Notice that v:Vec (length v) is subtle:
-- If length : Π n -> Vec n -> ℕ then actually this must be
-- v:Vec (length (length (length ...)) v)
-- What if we use intersection types though?
-- length : ∀ n -> Vec n -> ℕ
-- v:Vec (length v) -- Now typechecking is tricky (easily undecidable?)