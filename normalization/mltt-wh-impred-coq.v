...
match d with
| O => unit
| S d' => (instantiation d') * inst
end.
Fixpoint lookup (d : nat) : instantiation d -> fin d -> inst :=
match d return instantiation d -> fin d -> inst with
...
Inductive Psi {d} (rho : instantiation d) : tp d 0 -> inst -> Type :=
...
| Psi_all :
  ...
  (forall (R : inst) (pf : isCand R), @Psi (S d) (rho , R) B (psiB R pf))
...
Set Printing Universes.
(* Inductive Phi {d} (rho : instantiation d) : tp d 0 -> inst -> Type := *)
...
(*   (forall (R : inst) (pf : isCand R), @Phi (S d) (rho , R) B (phiB R pf)) *)
