...
rsubst : ctx -> ctx -> Set 
rsubst Δ Γ = ∀ {U} -> var Δ U -> rtm Γ U

sem : (Γ : ctx) -> (T : tp) -> Set
sem Γ (atom A) = rtm Γ (atom A)
sem Γ (T ⇝ S) = ∀ Δ -> rsubst Γ Δ -> sem Δ T → sem Δ S 
sem Γ (T × S) = sem Γ T * sem Γ S
sem Γ unit = Unit


ext : ∀ {Γ Δ T} -> rsubst Γ Δ -> rsubst (Γ , T) (Δ , T)
ext σ z = v z
ext σ (s y) = {!!}

mutual
 rappSubst : ∀ {Γ Δ S} -> rsubst Δ Γ -> rtm Δ S -> rtm Γ S
 rappSubst σ (v y) = σ y
 rappSubst σ (R · N) = rappSubst σ R · nappSubst σ N
 rappSubst σ (π₁ R) = π₁ (rappSubst σ R)
 rappSubst σ (π₂ R) = π₂ (rappSubst σ R)
 nappSubst : ∀ {Γ Δ S} -> rsubst Δ Γ -> ntm Δ S -> ntm Γ S 
 nappSubst σ (ƛ M) = ƛ (nappSubst (ext σ) M)
 nappSubst σ (neut R) = neut (rappSubst σ R)
 nappSubst σ < M , N > = < nappSubst σ M , nappSubst σ N >
 nappSubst σ tt = tt

_∘_ : ∀ {Δ Γ ψ} -> rsubst Δ Γ -> rsubst ψ Δ -> rsubst ψ Γ
(σ1 ∘ σ2) x = rappSubst σ1 (σ2 x)


id : ∀ {Γ} -> rsubst Γ Γ
id x = v x

appSubst : ∀ {Γ Δ} S -> rsubst Δ Γ -> sem Δ S -> sem Γ S
