...
sem unit Γ = Unit
sem (T + S) Γ = sum Γ (sem T) (sem S) (T + S)
...
isSheaf (T + T₁) s' f0 f1 = neut (case s' of reify f0 - reify f1) --Hmm... This seems to be able delaying the case analysis/reification until it's definitely needed...
...
eval (case M of N1 - N2) θ | inl y = eval N1 (extend θ y)
eval (case M of N1 - N2) θ | inr y = eval N2 (extend θ y)
eval (case M of N1 - N2) θ | neut y = isSheaf _ y (eval N1 (extend (λ x → appSubst _ wkn (θ x)) (reflect (v z)))) (eval N2 (extend (λ x → appSubst _ wkn (θ x)) (reflect (v z))))
