...
mutual
 data _,_⊢_-_ (θ : ctx type) (Γ : ctx type) : type -> judgement -> Set where
  ▹ : ∀ {A} -> (x : var Γ A)
            -> -------------------
               θ , Γ ⊢ A - true
  let-next : ∀ {A C J} (M : θ , Γ ⊢ (○ A) - true) (N : (θ , A) , Γ ⊢ C - J)
                   -> ---------------------------------------------------------------
                                          θ , Γ ⊢ C - J
  next : ∀ {A} -> (M : θ , Γ ⊢ A - next)
               -> --------------------------
  ...
  shift : ∀ {A} -> (M : ⊡ , θ ⊢ A - true)
                -> --------------------------
                     θ , Γ ⊢ A - next
  -- If the dia-next rule isn't a proper introduction rule then this isn't a proper elim rule
  let-box : ∀ {A C J} (M : θ , Γ ⊢ (□ A) - true) (N : (θ , □ A) , (Γ , A) ⊢ C - J)
                  -> ---------------------------------------------------------------
                                           θ , Γ ⊢ C - J
  box : ∀ {A Γ'} (M : θ , Γ ⊩ Γ' - true) (N : ⊡ , Γ' ⊢ A - true) (P : ⊡ , Γ' ⊩ Γ' - next)
              -> -------------------------------------------------------------------------------------
                                           θ , Γ ⊢ (□ A) - true
  dia-rec : ∀ {A C} (M : θ , Γ ⊢ (◇ A) - true) (N : ⊡ , (⊡ , A) ⊢ C - true) (P : (⊡ , C) , ⊡ ⊢ C - true)
                 -> ------------------------------------------------------------------------------------------------
                                           θ , Γ ⊢ C - true
  dia-now : ∀ {A} (M : θ , Γ ⊢ A - true)
               -> --------------------------
                    θ , Γ ⊢ (◇ A) - true
  dia-next : ∀ {A} (M : ⊡ , θ ⊢ (◇ A) - true)
               -> -------------------------------
                    θ , Γ ⊢ (◇ A) - true
  
 _,_⊩_-_ : (θ : ctx type) (Γ : ctx type) (Γ' : ctx type) -> judgement -> Set
 θ , Γ ⊩ Γ' - J = sub (λ A → θ , Γ ⊢ A - J) Γ'
