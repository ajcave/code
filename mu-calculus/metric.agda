...
agree2-trans (μ F) f n F' Ft ⟨ t ⟩ ⟨ u ⟩ ⟨ v ⟩ (acc rs) r1 r2 = agree2-trans F (extend f (μ⁺ F f)) n
  ...
  (extend' (λ x → (m : ℕ) (p : suc m ≤′ n) (t' u' v' : extend f (μ⁺ F f) x) →
        ...
        F' (λ m' x' x0 x1 → agree2 (μ F) f m' (λ x2 m0 z → F' x2 m0 (≤′-trans (≤′-step z) x')) x0 x1 (rs m' x'))
        x m p t' v')
     Ft (λ m p t' u' v' x x' → agree2-trans (μ F) f m (λ x0 m' x1 x2 x3 → F' x0 m' (≤′-trans (≤′-step x1) p) x2 x3) (λ x0 m' p' t0 u0 v0 x1 x2 → Ft x0 m' (≤′-trans (≤′-step p') p) t0 u0 v0 x1 x2) t' u' v' (rs m p) x x'))
  t u v (acc rs) r1 r2
agree2-trans (ν F) f n F' Ft ⟨ t ⟩ ⟨ u ⟩ ⟨ v ⟩ (acc rs) r1 r2 =  agree2-trans F (extend f (ν⁺ F f)) n
  (extend' (λ x → (m : ℕ) → m <′ n → extend f (ν⁺ F f) x → extend f (ν⁺ F f) x → Set)
     F' (λ m x x' x0 → agree2 (ν F) f m (λ x1 m' z → F' x1 m' (≤′-trans (≤′-step z) x)) x' x0 (rs m x)))
  (extend' (λ x → (m : ℕ) (p : suc m ≤′ n) (t' u' v' : extend f (ν⁺ F f) x) →
        extend' (λ x' → (m' : ℕ) → suc m' ≤′ n → extend f (ν⁺ F f) x' → extend f (ν⁺ F f) x' → Set)
        F' (λ m' x' x0 x1 → agree2 (ν F) f m' (λ x2 m0 z → F' x2 m0 (≤′-trans (≤′-step z) x')) x0 x1 (rs m' x'))
        x m p t' u' →
        extend' (λ x' → (m' : ℕ) → suc m' ≤′ n → extend f (ν⁺ F f) x' → extend f (ν⁺ F f) x' → Set)
        F' (λ m' x' x0 x1 → agree2 (ν F) f m' (λ x2 m0 z → F' x2 m0 (≤′-trans (≤′-step z) x')) x0 x1 (rs m' x'))
        x m p u' v' →
        ...
        F' (λ m' x' x0 x1 → agree2 (ν F) f m' (λ x2 m0 z → F' x2 m0 (≤′-trans (≤′-step z) x')) x0 x1 (rs m' x'))
        x m p t' v')
     Ft (λ m p t' u' v' x x' → agree2-trans (ν F) f m (λ x0 m' x1 x2 x3 → F' x0 m' (≤′-trans (≤′-step x1) p) x2 x3) (λ x0 m' p' t0 u0 v0 x1 x2 → Ft x0 m' (≤′-trans (≤′-step p') p) t0 u0 v0 x1 x2) t' u' v' (rs m p) x x'))
  (♭ t) (♭ u) (♭ v) (acc rs) r1 r2
